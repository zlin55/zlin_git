    ...
    "uploadError":function(index){
        ...
        if (wfu.showFileItemProgress) {
            ...
            for(var i=0;i<index.length;i++){
                $(progressObj[index[i]]).addClass("error");
            }
        ...
        let fileItemView = WpFileItemTools.getNeedUploadItemArray(uploadId);
        // 设置单个文件状态
        WpFileItemTools.getFileViewStatus(fileItemView).addClass("iconfont icon-gou");
        for(var i=0;i<index.length;i++){
            WpFileItemTools.getFileViewStatus($(fileItemView[index[i]])).addClass("iconfont icon-cha");
        }
        // 设置总进度条状态
        WpFileUploadTools.setProgressNumber(wfu,100);
    },
    /**
     * 上传成功
     */
